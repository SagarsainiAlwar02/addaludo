...
export default function Home() {
  ...
  // चेक करने के लिए कि कौन सा टैब एक्टिव है
  const isActive = (path) => location.pathname === path;

  return (
    <div className="page-container pt-1 relative min-h-screen pb-24 bg-[#f8f9fa]">
      ...
      {/* LIVE ANIMATED WHATSAPP SUPPORT BUTTON (BAR SE UPAR SET KIYA HAI) */}
      <div 
        onClick={() => navigate("/support")}
        className="fixed bottom-20 right-4 z-40 flex flex-col items-center cursor-pointer active:scale-90 transition-transform"
      >
        <div className="relative w-11 h-11 flex items-center justify-center">
          <div className="absolute inset-0 bg-[#25D366] rounded-full animate-ping opacity-25 duration-1000"></div>
          <div className="absolute bg-[#25D366] w-11 h-11 rounded-full shadow-lg flex items-center justify-center border border-white">
            <svg 
      ...
      {/* NEW FIXED BOTTOM NAVIGATION BAR (LIKE KHELSTAR) */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#2d3748] border-t border-gray-700 shadow-xl flex justify-around items-center h-14 px-2">
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center w-12 h-full transition-colors ${
            isActive("/wallet") ? "text-blue-400" : "text-gray-400"
          }`}
        >
          <i className="fa-solid fa-wallet text-lg"></i>
        </button>

        {/* TAB 3: REFER & EARN (PEOPLE ICON) */}
        <button 
          onClick={() => navigate("/refer")}
          className={`flex flex-col items-center justify-center w-12 h-full transition-colors ${
            isActive("/refer") ? "text-blue-400" : "text-gray-400"
          }`}
        >
        ...
        <button 
          onClick={() => navigate("/support")}
          className={`flex flex-col items-center justify-center w-12 h-full transition-colors ${
            isActive("/support") ? "text-blue-400" : "text-gray-400"
          }`}
        >
