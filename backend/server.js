...
//     try {
//   const message = `Hi Welcome to 5875 , OTP to yoconst smsUrl ur Login is ${otp} . Sender name - RESTPR`;
// const smsUrl = `https://login.msgbridge.in/sendSMS?username=ADDA&message=Hi%20Welcome%20to%20${otp}%20,%20OTP%20to%20your%20Login%20is%20${otp}%20.%20Sender%20name%20-%20RESTPR&sendername=RESTPR&smstype=TRANS&numbers=${phone}&apikey=434814a5-118b-4288-865d-f0f79c8e35cc`;
// const smsRes = await fetch(smsUrl);
// console.log("📲 MsgBridge SMS sent:", await smsRes.text());
//     } catch (smsErr) {
...
app.post("/api/otp/send", async (req, res) => {
  try {
    ...
    if (!process.env.TWOFACTOR_API_KEY) {
      console.log("WARNING: TWOFACTOR_API_KEY missing in .env. Operating in Sandbox/Development mode.");
