...
const Settings = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      const res = await API.get("/admin/settings-report");
      const data = getData(res);

      setBonusReport(Array.isArray(data?.bonus) ? data.bonus : []);
      setPenaltyReport(Array.isArray(data?.penalty) ? data.penalty : []);
    } catch (err) {
      console.log("Report Error:", getError(err));
      alert(getError(err));
    } finally {
  ...
  try {
    ...
    await API.post("/admin/bonus", payload);
...
const addPenalty = async () => {
    ...
    try {
      ...
      await API.post("/admin/penalty", payload);
